function [tvec, Savec, Sbvec, Scvec, Sdvec] = First_Scenario(check, cycles, C1, Po, zi, zc, I0, T0, h0, c0, tc, tgi, stay, home, ti, tg, tvec, Savec, Sbvec, Scvec, Sdvec, c, zopt)
    if check == 1
        for cycle = 1:cycles
            [z0, I0, T0, h0, c0, ~, t1, S1a, S1b, S1c, S1d] = Power_Control (Po, 1, ti, ti + tc, zi, I0, T0, h0, c0, "Time", 0, c);
            ...
            if z0 < zopt
                ...
                S6c = [];
                S6d = [];
                tf3 = tgi + tg + charging_time + stay;
            end
            [z0, I0, T0, h0, c0, ~, t7, S7a, S7b, S7c, S7d] = Power_Control (0, 0, tf3, tf3 + home, z0, I0, T0, h0, c0, "Time", 0, c);
            [~, ~, ~, ~, ~, ~, t8, S8a, S8b, S8c, S8d] = Power_Control (-1, 0, tf3 + home, zi, z0, I0, T0, h0, c0, "SOC", 0, c);
            ti = t8(end, 1);
            tg = tgi + t8(end, 1);
            tvec = [tvec; t1; t2; t3; t4; t5; t6; t7; t8];
            Savec = [Savec; S1a; S2a; S3a; S4a; S5a; S6a; S7a; S8a];
            Sbvec = [Sbvec; S1b; S2b; S3b; S4b; S5b; S6b; S7b; S8b];
            Scvec = [Scvec; S1c; S2c; S3c; S4c; S5c; S6c; S7c; S8c];
            Sdvec = [Sdvec; S1d; S2d; S3d; S4d; S5d; S6d; S7d; S8d];
        end
    else
        for cycle = 1:cycles
            [z0, I0, T0, h0, c0, ~, t1, S1a, S1b, S1c, S1d] = Power_Control (Po, 1, ti, ti + tgi, zi, I0, T0, h0, c0, "Time", 0, c);
