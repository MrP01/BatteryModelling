...
\begin{document}
  ...
  The graph data was retrieved using OSMnx \parencite{osmnx} which itself is built on NetworkX \parencite{networkx}. The edges retrieved from the OpenStreetMap API also contain data on the highest allowed velocity $v_{\rm max}$, but we did not use it in our solver as this would need to be a carefully considered constraint to the optimisation routine.
