...
def getModelParams(): # Should read from file but I am just copying from other program--parameterCurveFinding.py
                      # in nickWeek3
    R0 = np.array([0.99055269, -2.26465394,  1.93048217, -0.95348885,  0.52165477, -0.24699246, 0.08733394])
    R1 = np.array([24.93391031, -89.92504695, 130.05284213, -96.30378232, 38.4476705, -7.8265193, 0.64865373])
    C1 = np.array([-839658.33428892, 2121361.97592949, -2065305.40823765, 906760.09622885, -101216.29866021])
...
def computePowerAtGivenCycle(current, cycle, R0, R1, C1, OCV, operationBand=[1, 0.2]):
    ...
    V = lambda SOC: np.polyval(OCV, SOC) - current*np.polyval(R0, SOC) - current*np.polyval(R1, SOC)
    ...
    return current*integrate.quad(VTransformed, startTime, endTime)[0] # Why is this negative
