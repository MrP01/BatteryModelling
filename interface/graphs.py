...
class BatPlotCanvas(FigureCanvasQTAgg):
    def __init__(self, width=5, height=4) -> None:
...
class BatTimeseriesCanvas(BatPlotCanvas):
    ...
    def appendValueToLine(self, line, axes, time: float, value: float) -> None:
        XY = line.get_xydata()
        XY = np.append(XY[-self.maxValues :], ((time, value),), axis=0)
        line.set_xdata(XY[:, 0])
        line.set_ydata(XY[:, 1])

    def addMeasurement(self, measurement: BatteryMeasurement) -> None:
